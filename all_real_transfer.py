...
if __name__ == '__main__':
    ...
    epochs = 300
    test_batch = 1
    seeds = [1234,12345,] #1234567,12345678]
    # seeds = [1234]

    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print('deivice',device)

    # path = './rawFiles'
    # path = './compare'
    df= pd.read_csv('train_final.csv')
    df['Lab'] = df['Lab'].apply(lambda x: re.sub(r"'|,|\(|\)", "", x))  
    df['Lab'] = df['Lab'].apply(lambda x: '_'.join(x.split(' ')))
    
    parent_dir = './all_dataset'
    
    
    for lab in df['Lab'].unique():
    
      print('-------------------------------------------------------------------------------------------------------')
      print('the lab for this run is' , lab)
      print('size of the lab is, in case wrong', df[df['Lab']==lab].shape)
      each_dataset_dir = os.path.join(parent_dir, lab)
      print('single data path is located in', each_dataset_dir)
           
      path = each_dataset_dir #'./all_dataset/CFSRE'
              ...
              for fold in range(kfold):
                  ...
                  for epoch in tqdm(range(epochs),ncols=50):
                      if epoch % 50 == 49:
                          ...
                          model = torch.load( f'./all_models/{dataset_name}/fold{fold}.pkl')
                          new_lr = new_lr*0.1
                      ...
                      mae_test, medAE_test, mre_test, medRE_test = tester.test_regressor(test_loader)
                      if mae_test < best_test_mae:
                         ...
                         # r2_test_best = r2_test
                          torch.save(model, f'./all_models/{dataset_name}/fold{fold}.pkl')
  
                  print(f'time:{time.time()-time_start:.2f},dataset:{dataset_name},fold:{fold},mae:{best_test_mae:.2f},mre:{best_test_mre * 100:.2f},medAE:{best_test_medAE:.2f},medRE:{best_test_medRE * 100:.2f}')
                  file.write(f'{seed},{dataset_name},{fold},{best_test_mae},{best_test_mre * 100},{best_test_medAE:},{best_test_medRE * 100}\n')
                  file.flush()
      file.close()
